...
const CONTRACT_NAMES = ["FHECounter", "ExamVault"];
...
CONTRACT_NAMES.forEach((CONTRACT_NAME) => {
  console.log(`${line}Processing ${CONTRACT_NAME}...${line}`);
  ...
  if (deployLocalhost && deploySepolia) {
    if (
      ...
      console.warn(
        `${line}Deployments on localhost and Sepolia differ. Using localhost ABI for both networks. Consider re-deploying the contracts on both networks if this is unintended.${line}`
      );

      // Fallback: trust localhost ABI and override Sepolia ABI to keep the
      // generated TypeScript files consistent across networks.
